...
# Extract GSS Gun Control Dataset

Allen Downey

[MIT License](https://en.wikipedia.org/wiki/MIT_License)

This notebook extracts gun control-related variables from GSS data.
...
if match:
    ...
    OUTPUT_FILE = f"../data/interim/gss_gun_control_{year}_r{release}.hdf"
else:
    raise ValueError(f"Could not parse year and release from {INPUT_FILE}")

# Open log file now that we know year and release
os.makedirs('logs', exist_ok=True)
log_path = f'logs/gun_control_{year}_r{release}.txt'
debug_log = open(log_path, 'w')

# Write header information
debug_log.write("GSS Gun Control Extract Log\n")
debug_log.write("=" * 80 + "\n")
debug_log.write(f"Notebook: make_gun_control_extract.md\n")
...
if GIT_COMMIT:
    ...
    !git commit -m "Updating GSS gun control extract: {OUTPUT_FILE}"
    !git push
